...
class leapImageThread(threading.Thread):
    ...
    def __init__(self, source = 0, resolution = (640, 240), timeout=3.0):
        ...
        while(time.time() - self.timeoutTimer < self.timeout):
            if self.cam.isOpened():
                ...
                if(rval):
                    ...
                    frame = np.reshape(frame, (int(self.resolution[1]*1.5), self.resolution[0]*2))
                    # self.embeddedLine = self.getEmbeddedLine(frame)
                    leftRightImage = np.empty((2, int(self.resolution[1]*1.5), self.resolution[0]), dtype=np.uint8)
